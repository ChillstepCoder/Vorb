...
struct btTimings
{
    ...
    void flush()
    {
        for (int i = 0; i < m_numTimings; i++)
        {
            ...
            char startTimeRem1000Str[32];
            char endTimeRem1000Str[32];
...
unsigned long long getClockTimeNow() {
...
void MyEnterProfileZoneFunc(const char* msg)
{
	...
	gStartTimes[threadId][gStackDepths[threadId]] = getClockTimeNow();
...
void MyLeaveProfileZoneFunc()
{
	...
	unsigned long long int endTime = getClockTimeNow();
