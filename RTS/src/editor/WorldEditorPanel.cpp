...
void WorldEditorPanel::updateBuildingEdit() {
    ...
    if (mHitResult.didHit() && mBuildingEditState == BuildingEditState::CREATE) {
        ...
        GameThreadTasks::getInstance().addGenericTask([createPos, buildingId = mSelectedBuilding, dims = mPlotDimsDTile, world = mActiveWorld]() {
            ...
            const i32 approxZPos = world->getHeightmapGrid().getHeightAtVert<true>(createPos);
            BuildingBlueprintPtr bp = BuildingBlueprintGenerator::tryGenerateBlueprintSynchronous(
                buildingRepo.getLoadedOrUnloadedAsset(buildingId), 1.0f /*?*/, Cartesian::WEST, createPos, dims,
                ownedDTiles, BuildingBlueprintFlags(0), Random::getCachedRandom(), approxZPos
            );
            if (!bp) {
                assert(false);
                return;
    ...
    if (strength > 0.001f) {
        switch (editState) {
            ...
            case TerrainEditState::FLATTEN_TERRAIN:
                break;
            case TerrainEditState::DIRTY_ONLY:
                break;
            default:
                assert(false);
                break;
        }
        static_assert((int)TerrainEditState::COUNT == 4, "Update for new edit type");

        const f32 adjust = strength * brush.brushStrength;
        IHeightmapGrid& heightmapGrid = mActiveWorld->getHeightmapGrid();
        if (editState == TerrainEditState::DIRTY_ONLY) {
            heightmapGrid.markVertexDirty(id, vertPos.y * HEIGHTMAP_VERT_WIDTH_PER_PATCH + vertPos.x);
        }
        else {
