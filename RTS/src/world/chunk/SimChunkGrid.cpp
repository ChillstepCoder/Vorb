...
TileItemUID SimChunkGrid::tryDropItemStackOnGroundSimThread(ItemStack stack, TileCoord worldPos) {
    ...
    const TileCoord offset = worldPos - TileCoord(chunkCoord);
    TileItemUID uid = mChunkData[chunkId].tryAddItemStackToGroundAllowMerge(stack, offset.y * CHUNK_WIDTH + offset.x);
...
TileItemUID SimChunkGrid::connectItemEntityToGroundGameThreadNoMerge(ItemStack stack, f32v3 worldPos) {
    ASSERT_GAME_THREAD();
    const ChunkCoord chunkCoord = ChunkCoord::fromTilePos(worldPos);
    const ChunkID chunkId = chunkCoord.toGridIDType(mWidthChunks);
    const TileCoord offset = TileCoord(worldPos) - TileCoord(chunkCoord);
...
TileItemUID SimChunkGrid::connectItemEntityToGroundGameThreadAllowMerge(ItemStack stack, f32v3 worldPos) {
    ASSERT_GAME_THREAD();
    const ChunkCoord chunkCoord = ChunkCoord::fromTilePos(worldPos);
    const ChunkID chunkId = chunkCoord.toGridIDType(mWidthChunks);
    const TileCoord offset = TileCoord(worldPos) - TileCoord(chunkCoord);
...
void SimChunkGrid::untrackItem(TileItemUID uid, ItemID id, f32v3 worldPos) {
    const ChunkCoord chunkCoord = ChunkCoord::fromTilePos(worldPos);
    const ChunkID chunkId = chunkCoord.toGridIDType(mWidthChunks);
    const TileCoord offset = TileCoord(worldPos) - TileCoord(chunkCoord);
    mChunkData[chunkId].untrackItem(uid, id);
}
