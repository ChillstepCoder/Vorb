...
bool ModelMeshBuilder::buildStaticMesh(std::unique_ptr<Mesh>& mesh, MeshDrawMode drawMode) {
    return false;
}
...
bool ModelMeshBuilder::buildSkinnedMesh(
    ...
    if (fbxMesh->GetDeformerCount(FbxDeformer::eSkin) > 0) {
        ...
        outMesh.setData(mVerts.data(), (ui32)mVerts.size(), MeshDrawMode::STATIC);
