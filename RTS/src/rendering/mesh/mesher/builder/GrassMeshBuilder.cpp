...
void GrassMeshBuilder::createGrassMesh(GrassBillboardMesh& grassMesh, const Chunk& chunk, const ui32v2& tilePosStart, ui32 lod, const HeightmapPatchData* heightData)
{
    ...
    // d2 d3
    // d0 d1
    const auto interpolateDensity = [](f32 d0, f32 d1, f32 d2, f32 d3, const f32v2& offsetFromd0) -> f32 {
    ...
    const auto getOffsetFromD0 = [](f32 x, f32 y) -> f32v2 {
        f32v2 rv;
    ...
    const NoiseFunction& grassNoiseFunction = chunk.getWorld().getWorldGenerator().getGenerationData().mGrassNoise;
