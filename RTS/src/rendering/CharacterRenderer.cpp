...
void CharacterRenderer::render(vg::SpriteBatch& sb, const CharacterModel& model, const f32v2& pos, float angle) {
	...
	renderPart(sb, model.mBodyTextures[index], pos, bodyOffset, f32v2(0.0f), uvRect, BODY_SIZE, 0.3f);
	renderPart(sb, model.mFaceTextures[index], pos, bodyOffset, headOffset, uvRect, HEAD_SIZE, 0.6f);
	renderPart(sb, model.mHairTextures[index], pos, bodyOffset, headOffset, uvRect, HEAD_SIZE, 0.9f);
}
