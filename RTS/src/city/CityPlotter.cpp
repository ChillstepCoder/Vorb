...
CityPlot* CityPlotter::tryReservePlotForBuilding(const ui32v2& minPlotDims, const ui32v2& maxPlotDims)
{
    ...
    for (auto&& plot : mPlots) {
        ...
        // Min dims check
        if ((plot->aabb.width < minPlotDims.x || plot->aabb.depth < minPlotDims.y) ||
            (plot->aabb.width < minPlotDims.y || plot->aabb.depth < minPlotDims.x)) {
            continue;
        }

        // Max dims check
        if ((plot->aabb.width > maxPlotDims.x || plot->aabb.depth > maxPlotDims.y) ||
            (plot->aabb.width > maxPlotDims.y || plot->aabb.depth > maxPlotDims.x)) {
...
CityDistrict* CityPlotter::addDistrict(DistrictType type, CityDistrict* parent, ui32 size)
{
    ...
    if (!parent) {
        ...
        newDistrict->aabb.y = cityCenter.y - newDistrict->aabb.depth / 2;
    ...
    {
        const ui32v2 startPos(newDistrict->aabb.x, newDistrict->aabb.y + newDistrict->aabb.depth / 2);
        addRoad(*newDistrict,
            startPos,
    ...
    }
    {
        ...
        addRoad(*newDistrict,
            startPos,
            ui32v2(startPos.x, startPos.y + newDistrict->aabb.depth),
    ...
    // For each new plot, split them into 16 smaller
    {
        ...
        for (int numSplits = 0; numSplits < NUM_SPLITS; ++numSplits) {
            ...
            for (ui32 i = rootPlotIndex; i < stop; ++i) {
                ...
                const ui32v2 horizontalSplit = ui32v2(0, mPlots[i]->aabb.y + mPlots[i]->aabb.depth / 2);
...
bool CityPlotter::markDistrictTilesAsOwned(CityDistrict& district) {
    ...
    mCity.getWorld().efficientEnumTileAABB(district.aabb, [&wasConflict](Chunk& chunk, TileIndex tileIndex) {
        ...
        if (tile.hasFlag(TileFlags::IN_CITY)) {
            ...
            chunk.getTileContainer()->setTileFlag(tileIndex, TileFlags::IN_CITY);
        }
    });
...
bool CityPlotter::splitPlotByAABBIntersect(CityPlotIndex plotIndex, const i32AABB2& aabb, OPT CityRoad* road) {
    ...
    if (intersects[e_cast(CornerWinding::BOTTOM_LEFT)]) {
        ...
        const ui32v2 plotCorners[4] = {
            ...
            { plot.aabb.x, plot.aabb.y + plot.aabb.depth },
            { plot.aabb.x + plot.aabb.width, plot.aabb.y + plot.aabb.depth}
        };
