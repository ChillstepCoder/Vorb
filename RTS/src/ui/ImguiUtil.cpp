...
namespace ImguiUtil {
    ...
    // FROM HAZEL
    bool TreeNodeWithIcon(VGTexture icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint/* = IM_COL32_WHITE*/)
    {
        ...
        const ImU32 arrow_col = selected ? ImguiColors::Theme::backgroundDark : ImguiColors::Theme::muted;
        ...
        if (display_frame)
        {
            ...
            if (icon)
            {
                ...
                ImguiUtil::ShiftCursorY(-frame_height + pad);
                ImguiUtil::ShiftCursorX(arrowWidth);
                ImGui::Image((ImTextureID)icon, { frame_height - pad * 2.0f, frame_height - pad * 2.0f }, ImVec2(0, 0), ImVec2(1, 1), iconTint /*selected ? colourDark : tintFloat*/);
    ...
    bool TreeNodeWithIcon(VGTexture icon, const void* ptr_id, ImGuiTreeNodeFlags flags, ImColor iconTint, const char* fmt, ...)
    {
        va_list args;
        va_start(args, fmt);
        ImGuiWindow* window = ImGui::GetCurrentWindow();
        ...
        ImGuiContext& g = *GImGui;
        const char* label_end = g.TempBuffer.Data + ImFormatStringV(g.TempBuffer.Data, g.TempBuffer.size_in_bytes(), fmt, args);
        bool is_open = ImguiUtil::TreeNodeWithIcon(icon, window->GetID(ptr_id), flags, g.TempBuffer.Data, label_end, iconTint);

        va_end(args);
        return is_open;
    }

    bool TreeNodeWithIcon(VGTexture icon, const char* label, ImGuiTreeNodeFlags flags, ImColor iconTint /*= IM_COL32_WHITE*/)
    {
        ImGuiWindow* window = ImGui::GetCurrentWindow();
        if (window->SkipItems)
            return false;
