...
const float MIN_DISTANCE = 3.0f; // TODO: Matches NavigationComponent
const float DISTANCE_THRESHOLD = 3.0f; // TODO: Matches NavigationComponent=
...
inline void updateComponent(vecs::EntityID entity, UndeadAIComponent& cmp, TileGrid& world, EntityComponentSystem& ecs) {
	...
	// TODO: No allocations
	std::vector<EntityDistSortKey> nearbyEntities = world.queryActorsInRadius(myPhysCmp.mPosition, AGGRO_DISTANCE, ActorTypes::ACTORTYPE_HUMAN, true, entity);
	if (nearbyEntities.size()) {
		...
		if (distance > MIN_DISTANCE + DISTANCE_THRESHOLD) {
			...
			f32v2 offset = myPhysCmp.mPosition - targetPhysCmp.mPosition;
			...
			myNavCmp.mTargetPos = targetPhysCmp.mPosition + offset;
		}
	}
}
