...
vec3 lightPixel(vec3 pixelColor, vec3 normal, vec3 worldPos, vec2 screenUV, float roughness, float isSky, float shadow) {
    ...
    vec3 sunColor = getCurrentSunColor();
	...
	pixelColor += sunAngle * max(pow(hazeIntensity, 0.5) - 0.1, 0.0);
	...
	pixelColor = pixelColor * shadowMult * ShadowColor + pixelColor * (1.0 - shadowMult);
    ...
    float hazeDistance = length(hazeWorldPos) / hazeDiv + hazeDiv * isSky;
	float hazeDepth = pow(min(hazeDistance, 1.0), unHazeExponent);
	//float hazeDepth = min(pow(depth, 3500.0), 1.0);
	...
	// ==                     HDR TONE MAPPING            ==
	// =====================================================
    pixelColor = computeTonemapping(pixelColor);
    // Gamma correction
    pixelColor = pow(pixelColor, vec3(1.0 / unGamma));
