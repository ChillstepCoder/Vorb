...
void main() {
	...
	// Only isGround to stop sky ghosting
    fColor.rgb = isGround * texture(FboLight, fUV).rgb * texture(Fbo0, fUV).rgb;
	...
	fColor.rgb = fColor.rgb * (1.0 - depthHaze) + vec3(depthHaze) * sunTextureColor;
	...
	fColor.rgb = fColor.rgb + starsColor.rgb * starsColor.a * isSky * sparkle * starIntensity;
