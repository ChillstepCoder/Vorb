...
void main() {
    ...
    vec2 distortSample = texture(unSurfaceDistort, fUV.xy * unDistortTiling + uvOffset).rg * unSurfaceDistortAmount;
    // Add a finer detail layer (need to apply to normal too)
    //distortSample += texture(unSurfaceDistort, fUV.xy * unDistortTiling * 4.0 + uvOffset).rg * unSurfaceDistortAmount;
...
// ERROR READING AND WRITING TO SAME NORMAL TEXTURE
    ...
    vec2 normalSample = texture(unSurfaceDistort, fUV.xy * unDistortTiling + timeOffset + uvOffset).rg * unSurfaceDistortAmount;
